...
def extract_rows(df):
    ...
    metrics_map = {
        ...
        "acumulado en lo que va de año": "acumulado",
        "tasa de variación acumulada": "variacion_acumulada",
    }
    ...
    for i in range(df.shape[0]):
        ...
        if isinstance(cell, str):
            ...
            # 1) ¿Es fila de métrica? (OJO: aquí NO hacemos continue)
            if s_low in metrics_map:
                current_metric = metrics_map[s_low]
            else:
                # 2) ¿Es cabecera de país? (la siguiente fila suele ser "Dato base")
                nxt = df.iloc[i + 1, col0] if i + 1 < df.shape[0] else None
                if isinstance(nxt, str) and nxt.strip().lower() == "dato base":
                    current_country = s
                    current_metric = None
